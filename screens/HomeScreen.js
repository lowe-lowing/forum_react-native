    ...
    const unsubscribe = async () => {
      ...
      onSnapshot(q, (snaphot) => {
        ...
        snaphot.forEach(async (doc) => {
          ...
          const date =
            post.createdAt != null
              ? new Date(post.createdAt.seconds * 1000 - new Date().getTimezoneOffset() * 60 * 1000)
              : new Date();
          ...
          textObj.push(
            <View key={i} style={styles.postContainer}>
              <View style={{ flexDirection: "row" }}>
                ...
                >
                  <Image
                    source={post.authorPfp ? { uri: post.authorPfp } : require("../assets/icons/default_pfp.png")}
              ...
              {post.imageUrl && (
                <Image source={{ uri: post.imageUrl }} style={{ width: 100, height: 100, marginTop: 10 }} />
              )}
              <Text style={styles.messageText}>{post.message}</Text>
              <View style={{ flexDirection: "row" }}>
                <TouchableOpacity
                  onPress={() => {
                    navigation.navigate("Comments", docId);
                  }}
                >
